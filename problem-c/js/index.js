...
let opps = huskyGames2016.map(games => extractOpponent(games));
console.log(opps);
...
let losses = huskyGames2016.filter(games => huskiesLost(games));
console.log(losses);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
console.log(losses.map(games => extractOpponent(games)));
...
losses.forEach(games => console.log(games.opponent + " at " + games.home + ", " + games.opponent_score + " to " + games.home_score));
...
function didHuskyFumble(game) {
  return game.fumbles > 0;
}
console.log(huskyGames2016.filter(games => didHuskyFumble(games)).length);
...
function mostYardsPassing(first, second) {
  if (!first.passing_yards) {
    return second;
  } else if (first.passing_yards > second.passing_yards) {
    return first;
  } else {
    return second;
  }
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, didHuskyFumble);
...
let fumbleLosses = huskyGames2016.filter(games => fumbledAndLostFilter(games));
